-- Location: supabase/migrations/20250906053554_fix_assistant_state_constraints.sql
...
-- Description: Fix assistant state constraints and ensure consistent string values (Active/Inactive) throughout the system

-- Fix 1: Remove problematic check constraint that's causing violations
...
-- Based on sample data, the application expects 'Active'/'Inactive' string values
...
COMMENT ON MIGRATION IS 'Fix assistant state constraints and ensure consistent string values (Active/Inactive) throughout the system';